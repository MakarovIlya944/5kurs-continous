...
class Spline():
    ...
    def __init__(self, file, _K, _paint_K=10):
        ...
        if dim == 1:
            ...
            points = np.meshgrid(self.netX, self.netY)
            f_i = lambda ind: [ind, ind + 1, ind + kn[0], ind + kn[0] + 1]
            for i in range(K[1]):
                for j in range(K[0]):
                    ind = i*kn[0] + j
                    self.net.append(np.array([points[0][i][j],points[1][i][j]]))
                    self.elements.append(Element(self.net[ind], f_i(ind)))
                self.net.append(np.array([points[0][i][K[0]],points[1][i][K[0]]]))
            for j in range(len(self.netX)):
                self.net.append(np.array([points[0][K[1]][j],points[1][K[1]][j]]))
